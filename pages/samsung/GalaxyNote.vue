<template>
  <Head>
    <Title>Samsung - Galaxy Note Series</Title>
  </Head>
...
import { useProductStore } from "~~/store/samsungGalaxyNote";

let productList = computed(() =>
  useProductStore().data.filter((item) => item.type === "Samsung Galaxy Note")
);
let search = ref(""),
  dataToShow = ref();
